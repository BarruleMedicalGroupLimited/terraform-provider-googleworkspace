...
See [`go.mod`](../go.mod) for which version of Go to use while developing the provider. You can manage it automatically using [`goenv`](https://github.com/syndbg/goenv).
...
Configuring tests is similar to configuring the provider; see the [Provider Configuration Reference](../docs/index.md) for more details. The following environment variables must be set in order to run tests:
...
2. Iterate through the list and determine if a resource is [sweepable](https://github.com/BarruleMedicalGroupLimited/terraform-provider-googleworkspace/blob/main/internal/provider/googleworkspace_sweeper_test.go#L22)
...
This provider is not published on the public Terraform Registry, so pick a
provider source address for local use, e.g.
`registry.terraform.io/barrulemedicalgrouplimited/googleworkspace`, and use
that same address both in your `required_providers` block and in the setup
below.

Setup - the symlink target and the `filesystem_mirror` path below must be
the *same* directory, or Terraform won't find the binary. This derives your
OS/arch and Go bin dir rather than hardcoding them:
```bash
OS_ARCH="$(go env GOOS)_$(go env GOARCH)"
BIN_DIR="$(go env GOBIN)"
BIN_DIR="${BIN_DIR:-$(go env GOPATH)/bin}"
PLUGIN_DIR=~/.terraform.d/plugin-cache/registry.terraform.io/barrulemedicalgrouplimited/googleworkspace/5.0.0/"${OS_ARCH}"

mkdir -p "$PLUGIN_DIR"
ln -s "$BIN_DIR/terraform-provider-googleworkspace" "$PLUGIN_DIR/terraform-provider-googleworkspace_v5.0.0"
```
...
provider_installation {
	filesystem_mirror {
		...
		include = ["registry.terraform.io/barrulemedicalgrouplimited/googleworkspace"]
	}
}
```
(Don't also set a top-level `plugin_cache_dir` to this same path - Terraform
uses that for a different purpose, the download cache, and the two aren't
meant to overlap.)
...
```bash
rm -rf ~/.terraform.d/plugin-cache/registry.terraform.io/barrulemedicalgrouplimited/
```
...
Signing requires a repository secret that isn't set by default:
...
- `PASSPHRASE` - the passphrase for that key, if it's passphrase-protected.
  Not required for a passphrase-less key; when it is required and unset,
  the "Import GPG key" step of the release workflow fails before GoReleaser
  ever runs.

Set whichever of these your key needs under the repository's Settings ->
Secrets and variables -> Actions.

### Migrating existing state from the upstream provider address

If you (or anyone else) have Terraform state created against the real
`registry.terraform.io/hashicorp/googleworkspace`, switching a config's
`required_providers` block to this fork's address
(`registry.terraform.io/barrulemedicalgrouplimited/googleworkspace`) will
make Terraform treat existing resources as managed by an unrecognized
provider. Run, once per state, before `terraform init -upgrade`:

```bash
terraform state replace-provider registry.terraform.io/hashicorp/googleworkspace registry.terraform.io/barrulemedicalgrouplimited/googleworkspace
```

There's no migration needed for state that was always created against this
fork.
